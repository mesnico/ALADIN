...
class RetrievalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_name, im_feature, od_labels = self.get_image_and_od_labels(index)
        # caption = self.captions[cap_idxs[0]][cap_idxs[1]]
        # od_labels = self.get_od_labels(img_key)
        # example = self.tensorize_example(caption, feature, text_b=od_labels)
        label = 1 # if img_key == cap_idxs[0] else 0
        return index, im_name, im_feature, od_labels #tuple(list(example) + [label])

    def get_image_and_od_labels(self, image_idx):
        ...
        features = np.stack([np.frombuffer(base64.b64decode(o['feature']), dtype=np.float32) for o in objects])
        pos_feat = generate_additional_features(labels['boxes'], results["image_h"], results["image_w"])
        features = np.hstack((features, pos_feat))
        t_features = torch.from_numpy(features)
        od_labels = ' '.join(labels['class'])
        return im_name, t_features, od_labels
...
def generate_additional_features(rect, h, w):
    ...
    res = np.concatenate((rect, np.expand_dims(rect[:, 3]-rect[:, 1], axis=1), np.expand_dims(rect[:, 2]-rect[:, 0], axis=1)), axis=1)
...
class MyCollate:
    ...
    def __call__(self, batch):
        index, im_names, im_feature, od_labels = zip(*batch)
        ...
        return index, im_names, examples_imgs
