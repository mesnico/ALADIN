...
def evalrank(config, checkpoint, split='dev', fold5=False, eval_mode='alignment', auxiliary_evaluation=True, dataloader='legacy', data_split_factor=10):
    """
    ...
    if dataloader == 'legacy':
        data_loader = get_test_loader(config, workers=4, split_name=split)
    elif dataloader == 'speaksee':
        image_field = ImageDetectionsField(detections_path=config['image-model']['pre-extracted-features-root'],
                                           max_detections=36, load_in_tmp=False,
                                           sort_by_prob=True)

        dataset = COCO(image_field, RawField(), config['dataset']['images-path'],
                       '{}/coco/annotations'.format(config['dataset']['data']),
                       '{}/coco/annotations'.format(config['dataset']['data']), cut_validation=True)
        _, _ , test_dataset = dataset.splits

        speaksee_collate = data.SpeakseeCollate(config)
        data_loader = DataLoader(test_dataset, batch_size=config['training']['bs'], shuffle=False,
                                 num_workers=4, collate_fn=speaksee_collate)
    ...
    if eval_mode == 'alignment':
        sim_matrix_fn = AlignmentContrastiveLoss(aggregation=config['training']['alignment-mode'], return_similarity_mat=True)
    elif eval_mode == 'crossattention':
        def cross_att_sim_fn(img, cap, img_len, cap_len):
            with torch.no_grad():
                scores = torch.zeros(len(img), len(cap)).cuda()
                # for i, c in itertools.product(range(len(img)), range(len(cap))):
                #     img_emb, cap_emb = model.img_txt_enc.cross_attention_aggregation(img[i, :img_len[i]].unsqueeze(0), cap[c, :cap_len[c]].unsqueeze(0), [img_len[i]], [cap_len[c]])
                #     score = torch.nn.functional.cosine_similarity(img_emb.squeeze(0), cap_emb.squeeze(0), dim=0)
                #     scores[i, c] = score

                # optimized code
                num_imgs = len(img)
                num_caps = len(cap)
                if num_imgs < num_caps:
                    for i in range(num_imgs):
                        img_ext = img[i].unsqueeze(0).expand(num_caps, -1, -1)
                        img_ext_len = [img_len[i]] * num_caps
                        img_emb, cap_emb = model.img_txt_enc.cross_attention_aggregation(img_ext[:, :max(img_ext_len)], cap[:, :max(cap_len)], img_ext_len, cap_len)
                        scores[i, :] = torch.nn.functional.cosine_similarity(img_emb, cap_emb, dim=1)
                else:
                    for c in range(num_caps):
                        cap_ext = cap[c].unsqueeze(0).expand(num_imgs, -1, -1)
                        cap_ext_len = [cap_len[c]] * num_imgs
                        img_emb, cap_emb = model.img_txt_enc.cross_attention_aggregation(img[:, :max(img_len)], cap_ext[:, :max(cap_ext_len)], img_len, cap_ext_len)
                        scores[:, c] = torch.nn.functional.cosine_similarity(img_emb, cap_emb, dim=1)
            return scores
    ...
    if auxiliary_evaluation:
        ...
        recall_1k_5fold_test(img_embs[:, 0, :], cap_embs[:, 0, :])
    ...
    if not fold5:
        ...
        r, rt = i2t(img_embs, cap_embs, img_lenghts, cap_lenghts, return_ranks=True, ndcg_scorer=ndcg_val_scorer, sim_function=sim_matrix_fn, cap_batches=25 * data_split_factor)
        ri, rti = t2i(img_embs, cap_embs, img_lenghts, cap_lenghts, return_ranks=True, ndcg_scorer=ndcg_val_scorer, sim_function=sim_matrix_fn, im_batches=5 * data_split_factor)
        ar = (r[0] + r[1] + r[2]) / 3
        ...
        for i in range(5):
            r, rt0 = i2t(img_embs[i * 5000:(i + 1) * 5000], cap_embs[i * 5000:(i + 1) * 5000],
                         ...
                         return_ranks=True, ndcg_scorer=ndcg_val_scorer, fold_index=i, sim_function=sim_matrix_fn, cap_batches=5 * data_split_factor)
            print("Image to text: %.1f, %.1f, %.1f, %.1f, %.1f, ndcg_rouge=%.4f ndcg_spice=%.4f" % r)
            ri, rti0 = t2i(img_embs[i * 5000:(i + 1) * 5000], cap_embs[i * 5000:(i + 1) * 5000],
                           img_lenghts[i * 5000:(i + 1) * 5000], cap_lenghts[i * 5000:(i + 1) * 5000],
                           return_ranks=True, ndcg_scorer=ndcg_val_scorer, fold_index=i, sim_function=sim_matrix_fn, im_batches=1 * data_split_factor)
            if i == 0:
                rt, rti = rt0, rti0
            print("Text to image: %.1f, %.1f, %.1f, %.1f, %.1f, ndcg_rouge=%.4f, ndcg_spice=%.4f" % ri)
            ar = (r[0] + r[1] + r[2]) / 3
