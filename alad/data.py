...
class BottomUpFeaturesDataset:
    ...
    def __getitem__(self, index):
        ...
        img_feat = np.load(img_feat_path)['feat'][:36]
        img_boxes = np.load(img_box_path)[:36]
...
class Collate:
    ...
    def __call__(self, data):
        ...
        if not preextracted_images:
            ...
            return img_features, targets, feat_lengths, cap_lengths, out_boxes, ids, img_ids


class SpeakseeCollate:
    def __init__(self, config):
        self.vocab_type = config['text-model']['name']
        if self.vocab_type == 'bert':
            self.tokenizer = BertTokenizer.from_pretrained(config['text-model']['pretrain'])
    ...
    def __call__(self, data):
        ...
        feats, boxes, imgs_ids = zip(*images)
        ...
        # Merge images (convert tuple of 3D tensor to 4D tensor)
        # they are image features, variable length
        feat_lengths = [(f != 0).any(axis=1).sum() + 1 for f in feats]  # +1 because the first region feature is reserved as CLS
        feat_dim = feats[0].shape[1]
        # feats = [torch.Tensor(f) for f in feats]
        # feats = torch.stack(feats, dim=0)
        # img_features = torch.cat((torch.zeros(len(images), 1, feat_dim), torch.Tensor(feats)), dim=1) # insert CLS at the beginning
        img_features = torch.zeros(len(images), max(feat_lengths), feat_dim)
        for i, img in enumerate(feats):
            end = feat_lengths[i]
            img_features[i, 1:end] = torch.Tensor(img)[:end-1, :]

        # boxes = [torch.Tensor(b)[:, :4] for b in boxes]
        # boxes = torch.stack(boxes, dim=0)
        # out_boxes = torch.cat((torch.zeros(len(images), 1, 4), torch.Tensor(boxes)), dim=1) # insert CLS at the beginning
        box_lengths = feat_lengths
        out_boxes = torch.zeros(len(boxes), max(box_lengths), 4)
        for i, box in enumerate(boxes):
            end = box_lengths[i]
            out_boxes[i, 1:end] = torch.Tensor(box)[:end-1, :]
        ...
        # features = features.permute(0, 2, 1)
        return img_features, targets, feat_lengths, cap_lengths, out_boxes, None, imgs_ids
